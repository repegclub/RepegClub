...
use crate::state::{Config, RoundStatus, Week, CONFIG, STATE, WEEKS, WINNER_INDEX};
...
pub fn open_new_week(storage: &mut dyn Storage, env: &Env, week_id: u64) -> StdResult<()> {
    let week = Week {
        week_id,
...
/// week was drawn - no admin discretion, no live redirection of an active
/// prize. Sweeps whatever's left to the treasury; any legitimate
/// wallet-recovery claim is handled off-chain by the (multisig) treasury.
pub fn execute_sweep_expired_prize(
    ...
    let swept_amount = week.prize_remaining;
    let winner = week.winner.clone();
    week.prize_remaining = Uint128::zero();
    WEEKS.save(deps.storage, week_id, &week)?;
