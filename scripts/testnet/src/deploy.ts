...
const WASM_PATH = path.resolve(
  __dirname,
  "../../../contracts/wheel-test-min/artifacts/wheel_test_min.wasm"
);
...
async function main() {
  ...
  console.log(`Storing code (${wasmByteCode.length} bytes)...`);
  ...
  if (!codeIdAttr) {
    throw new Error("code_id not found in store_code tx events");
  }
  const codeId = BigInt(codeIdAttr.value);
  console.log(
    `Code ID: ${codeId} | tx: ${storeRes.txResponse.txhash} | gasUsed: ${storeRes.txResponse.gasUsed}`
  );
  ...
  const instRes = await admin.broadcastTxSync({
    msgs: [
      new MsgInstantiateContract({
        ...
        codeId,
        label: "wheel-test-min",
        msg: { ticket_denom: "uluna" },
        funds: [],
      }),
    ],
  });
  ...
  if (!addrAttr) {
    throw new Error("contract address not found in instantiate tx events");
  }
  const contractAddress = addrAttr.value;
  console.log(
    `Contract address: ${contractAddress} | tx: ${instRes.txResponse.txhash} | gasUsed: ${instRes.txResponse.gasUsed}`
  );
  ...
  console.log("Saved deployment info to", DEPLOYMENT_PATH);
}
